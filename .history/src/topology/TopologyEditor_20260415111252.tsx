  ...
  return (
    ...
    >
      ...
      <div style={{ borderRadius: 12, overflow: 'hidden' }}>
        ...
        <div style={{ padding: '10px 13px 13px' }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: s.bg, border: `1px solid ${s.border}`, borderRadius: 20, padding: '2px 9px', marginBottom: 10 }}>
            {data.status === 'online' ? <Wifi size={10} style={{ color: s.color }} />
              : data.status === 'offline' ? <WifiOff size={10} style={{ color: s.color }} />
              : data.status === 'partial' ? <Activity size={10} style={{ color: s.color }} />
      ...
      <style>{`
        ...
        @keyframes pulseRed { 0%,100%{box-shadow:0 0 18px rgba(239,68,68,0.55),inset 0 1px 0 rgba(255,255,255,0.04)} 50%{box-shadow:0 0 32px rgba(239,68,68,0.85),inset 0 1px 0 rgba(255,255,255,0.04)} }
        @keyframes pulseAmber { 0%,100%{box-shadow:0 0 18px rgba(245,158,11,0.40),inset 0 1px 0 rgba(255,255,255,0.04)} 50%{box-shadow:0 0 28px rgba(245,158,11,0.70),inset 0 1px 0 rgba(255,255,255,0.04)} }
      `}</style>
    </div>
  );
};
  ...
  const processNode = (node: TreeNode, x: number, y: number, parentId?: string): string => {
    ...
    if (parentId) {
      ...
      edges.push({
        ...
        markerEnd: { type: MarkerType.ArrowClosed, color: s.color, width: 28, height: 28 },
        animated: isOnline,
      });
    }
...
const DebugPanel = ({ logs, onClear }: { logs: DebugEntry[]; onClear: () => void }) => {
    ...
    >
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#60a5fa' }}>
          <Bug size={12} />
          <span style={{ fontWeight: 700, fontSize: 11, letterSpacing: 0.5 }}>DEBUG PANEL</span>
          <span style={{ color: '#475569', fontSize: 10 }}>({logs.length} events)</span>
        </div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
            onClick={e => { e.stopPropagation(); onClear(); }}
            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 4, color: '#64748b', cursor: 'pointer', padding: '1px 7px', fontSize: 10 }}
          >
            clear
          </button>
          {open ? <ChevronDown size={12} color="#64748b" /> : <ChevronUp size={12} color="#64748b" />}
        </div>
      </div>

      {/* Log lines */}
      {open && (
        <div ref={scrollRef} style={{ overflowY: 'auto', maxHeight: 272, padding: '6px 0' }}>
          {logs.length === 0 && (
            <div style={{ color: '#334155', padding: '6px 10px' }}>No events yet…</div>
          )}
          {logs.map((entry, i) => (
            <div key={i} style={{ padding: '2px 10px', display: 'flex', gap: 8, alignItems: 'flex-start', borderBottom: '1px solid rgba(255,255,255,0.02)' }}>
              ...
              <span style={{ color: '#94a3b8', flex: 1, lineHeight: 1.5 }}>
                {entry.message}
                {entry.detail && (
                  <span style={{ color: '#475569', display: 'block', fontSize: 10, marginTop: 1 }}>{entry.detail}</span>
                )}
              </span>
            </div>
          ))}
...
const TopologyEditor = () => {
  ...
  /* ─── Debug logger ─── */
  const log = useCallback((tag: string, color: string, message: string, detail?: string) => {
    const time = new Date().toLocaleTimeString('en', { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
    console.log(`[${tag}] ${message}`, detail ?? '');
    setDebugLogs(prev => [...prev.slice(-99), { time, tag, color, message, detail }]);
  }, []);

  const normalizeArea = (value?: string) => value?.trim().toLowerCase() ?? '';

  /* ─── Extract unique areas from tree ─── */
  const extractAreas = useCallback((treeNodes: TreeNode[]): string[] => {
    const areaMap = new Map<string, string>();
    const traverse = (node: TreeNode) => {
      const value = node.area?.trim();
      const key = value?.toLowerCase() ?? '';
      if (value && key && !areaMap.has(key)) areaMap.set(key, value);
  ...
  /* ─── Filter tree by area ─── */
  const filterTreeByArea = useCallback((treeNodes: TreeNode[], targetArea: string): TreeNode[] => {
    if (targetArea === 'all') return treeNodes;
    const targetKey = normalizeArea(targetArea);

    log('FILTER', '#f59e0b', `Filtering for area key: "${targetKey}"`, `Total root nodes: ${treeNodes.length}`);

    // Collect sample of area values seen in tree for comparison
    const seenAreas: string[] = [];
    const collectAreas = (node: TreeNode) => {
      const k = normalizeArea(node.area);
      if (!seenAreas.includes(k)) seenAreas.push(k);
      node.children?.forEach(collectAreas);
    };
    treeNodes.forEach(collectAreas);
    log('FILTER', '#f59e0b', `Area keys in tree: [${seenAreas.slice(0, 10).map(a => `"${a}"`).join(', ')}${seenAreas.length > 10 ? '…' : ''}]`);
    ...
    const result = treeNodes.map(traverse).filter((n): n is TreeNode => n !== null);
    log('FILTER', '#f59e0b', `Filter result: ${result.length} root nodes after filter`);
    return result;
  }, [log]);

  /* ─── Convert Location to TreeNode ─── */
  const processTreeData = useCallback((locs: Location[], level = 0): TreeNode[] =>
    locs.map((node) => ({
      ...node,
      area: node.area?.trim() ?? '',
  ...
  const fetchTopology = useCallback(async () => {
    ...
    try {
      ...
      log('FETCH', '#60a5fa', `GET ${url}`);

      const response = await fetch(url, { headers: getAuthHeaders() });
      log('FETCH', '#60a5fa', `Response: ${response.status} ${response.statusText}`);
      ...
      log('PARSE', '#a78bfa', `Response shape: ${isArray ? 'array' : 'object'}`, `Top-level keys: ${Object.keys(data).join(', ')}`);

      if (isArray) {
        log('PARSE', '#a78bfa', `Root array length: ${treeData.length}`);
        if (treeData[0]) {
          log('PARSE', '#a78bfa', `First root node: id=${treeData[0].id}, name="${treeData[0].name}", area="${treeData[0].area}"`,
            `children count: ${(treeData[0].children || []).length}`);
        }
      } else {
        log('PARSE', '#a78bfa', `Root node: id=${treeData.id}, name="${treeData.name}", area="${treeData.area}"`,
          `children count: ${(treeData.children || []).length}`);
      }

      const processed = isArray
      ...
      countNodes(processed);
      log('PARSE', '#a78bfa', `Processed tree: ${processed.length} root(s), ${totalNodes} total nodes`);

      setAllTreeData(processed);

      const areas = extractAreas(processed);
      log('AREAS', '#34d399', `Found ${areas.length} distinct areas: [${areas.map(a => `"${a}"`).join(', ')}]`);
      setAvailableAreas(areas);

      if (fetchId !== fetchIdRef.current) {
        log('FETCH', '#60a5fa', 'Stale fetch — discarding result');
        return;
      }
      setLoading(false);
      log('FETCH', '#60a5fa', 'Data loaded successfully ✓');
    } catch (err) {
      if (fetchId !== fetchIdRef.current) return;
      const msg = err instanceof Error ? err.message : 'Failed to load topology';
      log('ERROR', '#ef4444', msg);
      setError(msg);
      setLoading(false);
    }
  }, [log, processTreeData, extractAreas]);

  /* ─── Apply area filter and build graph ─── */
  useEffect(() => {
    if (allTreeData.length === 0) {
      log('GRAPH', '#ec4899', 'Skipping graph build — allTreeData is empty');
      return;
    }

    log('GRAPH', '#ec4899', `Building graph — selectedArea="${selectedArea}", allTreeData roots: ${allTreeData.length}`);

    const filtered = filterTreeByArea(allTreeData, selectedArea);

    if (filtered.length > 0) {
      const input = filtered.length === 1 ? filtered[0] : filtered;
      const { nodes: gn, edges: ge } = buildGraphData(input);
      log('GRAPH', '#ec4899', `Graph built: ${gn.length} nodes, ${ge.length} edges`);
      setNodes(gn);
      setEdges(ge);
    } else {
      log('GRAPH', '#ec4899', 'Filter returned 0 nodes — clearing graph');
      setNodes([]);
      setEdges([]);
    }
  }, [selectedArea, allTreeData, setNodes, setEdges, filterTreeByArea, log]);

  useEffect(() => {
    if (!reactFlowInstance || nodes.length === 0) return;
    reactFlowInstance.fitView({ padding: 0.2 });
  }, [reactFlowInstance, nodes.length, selectedArea]);
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onClick={fetchTopology} disabled={loading} title="Refresh"
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <select value={selectedArea} onChange={(e) => {
            log('UI', '#38bdf8', `Area changed → "${e.target.value}"`);
            setSelectedArea(e.target.value);
          }}
            style={{ background: '#0f172a', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '7px 12px', fontSize: 13, color: '#e2e8f0', cursor: 'pointer', outline: 'none', minWidth: 140, appearance: 'none', WebkitAppearance: 'none', MozAppearance: 'none' }}>
            <option value="all" style={{ background: '#0f172a', color: '#e2e8f0' }}>All Areas ({allTreeData.length})</option>
      ...
      <div style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
        ...
        {!loading && !error && nodes.length === 0 && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 10 }}>
            ...
            <p style={{ color: '#475569', fontSize: 14, margin: 0 }}>No locations found for selected area</p>
          </div>
        )}
        ...
        >
          ...
          <MiniMap nodeColor={(n) => getStatus(n.data?.status).color} maskColor="rgba(6,13,26,0.75)"
...
const NodeDetails: React.FC<NodeDetailsProps> = ({ nodeId, nodes, onClose }) => {
  ...
  return (
    <div style={{ position: 'absolute', top: 16, right: 16, bottom: 16, width: 300, background: 'rgba(10,18,32,0.96)', border: `1px solid ${s.border}`, borderRadius: 14, backdropFilter: 'blur(16px)', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: `${s.glow}, 0 24px 48px rgba(0,0,0,0.6)`, zIndex: 30, fontFamily: "'DM Sans', 'Inter', sans-serif" }}>
        ...
        {d.device_count !== undefined && (
          <Row label="Devices" value={
            <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
              {[{ l: 'Total', v: d.device_count, c: '#94a3b8' }, { l: 'Online', v: d.online_device_count ?? 0, c: '#22c55e' }, { l: 'Offline', v: d.offline_device_count ?? 0, c: '#ef4444' }].map(({ l, v, c }) => (
