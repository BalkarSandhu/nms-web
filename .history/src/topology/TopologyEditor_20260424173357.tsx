...
const TopoCanvas: React.FC<CanvasProps> = ({ nodes, links, selectedId, onSelect }) => {
  ...
  // FIX 2: Track whether the user has manually zoomed/panned so we never auto-fit again
  const userHasZoomedRef = useRef(false);
  // FIX 2: Track whether we've done the very first auto-fit
  ...
  useEffect(() => {
    ...
    const zoom = d3.zoom<SVGSVGElement, unknown>()
      ...
      .on('zoom', e => {
        ...
        // FIX 2: Mark that the user has interacted with zoom/pan
        // sourceEvent is null for programmatic zoom, set by d3 for user gestures
        if (e.sourceEvent) {
    ...
    svg.on('dblclick', () => {
      // Reset zoom on double-click and clear the user-zoomed flag so next data load fits
  ...
  useEffect(() => {
    ...
    // FIX 2: Only auto-fit the view on the VERY FIRST load, never again after user zooms
    sim.on('end', () => {
      // Skip auto-fit entirely if user has already interacted with zoom/pan
      if (userHasZoomedRef.current) return;
      // Also skip if we've already done the initial fit (e.g. on status-only refresh)
...
const TopologyGraph: React.FC = () => {
  ...
  // FIX 1: Use refs for the fetch parameters so the interval never needs to be recreated.
  // The interval always reads the latest values without being a dependency.
  ...
  // FIX 1: Build the API URL from refs so this function is stable (no deps that change)
  const buildUrl = useCallback(() => {
    const vm = viewModeRef.current;
    const lid = locationIdRef.current;
    return vm === 'subtree' && lid
      ? `${import.meta.env.VITE_NMS_HOST}/locations/${lid}/subtree`
      : `${import.meta.env.VITE_NMS_HOST}/locations/tree`;
  }, []); // stable — no deps

  // FIX 1: fetchTopology is now stable. It only changes when buildUrl changes (never).
  ...
  // FIX 1: Only re-fetch when the user explicitly changes view parameters, not on every render.
  // We track a "fetch trigger" counter that bumps only when something meaningful changes.
  const [fetchTrigger, setFetchTrigger] = useState(0);
  const triggerFetch = useCallback(() => setFetchTrigger(c => c + 1), []);

  // Initial load + whenever the user changes viewMode / locationId / selectedArea
  useEffect(() => { fetchTopology(); }, [fetchTrigger, fetchTopology]);

  // Re-trigger fetch when user changes these filter controls
  useEffect(() => { triggerFetch(); }, [viewMode, locationId, selectedArea]);

  // FIX 1: Auto-refresh interval — only one interval, only patches status, never recreated
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        ...
        // Only patch status fields — do NOT rebuild nodes/links (avoids re-running the sim)
  ...
  }, [autoRefresh, buildUrl]); // buildUrl is stable, so this effect only re-runs when autoRefresh toggles
