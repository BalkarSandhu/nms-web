...
const TopologyEditor = () => {
  ...
  /**
   * FIXED: filterTreeByArea now properly reconstructs the tree hierarchy
   * when filtering by a specific area
   */
  const filterTreeByArea = (treeNodes: TreeNode[], area: string): TreeNode[] => {
    ...
    const filterNode = (node: TreeNode): TreeNode | null => {
      ...
      // Include node if:
      // 1. It matches the selected area, OR
      // 2. It has children that match (to preserve hierarchy)
      if (matchesArea || filteredChildren.length > 0) {
        return {
          ...node,
  ...
  /**
   * FIXED: Fetch all locations using page_size=500 instead of tree endpoint
   * when in tree view mode, then build hierarchy using parent_id references
   */
  const fetchTopology = useCallback(async () => {
    try {
      ...
      if (viewMode === 'subtree' && locationId) {
        ...
        // Full tree mode: fetch all locations with page_size=500
        const queryParams = new URLSearchParams({
          page_size: '500', // Fetch all locations
        });
        const url = `${import.meta.env.VITE_NMS_HOST}/locations?${queryParams}`;
        const response = await fetch(url, { headers: getAuthHeaders() });
        if (!response.ok) throw new Error(`API error ${response.status}: ${response.statusText}`);
        ...
        // Build tree hierarchy from flat list using parent_id
        ...
        locations.forEach((loc: Location) => {
          ...
          locationMap.set(loc.id, treeNode);
        });
      ...
      setAvailableAreas(extractAreas(treeData));
