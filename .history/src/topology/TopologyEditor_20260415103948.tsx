...
const TopologyEditor = () => {
  ...
  const filterTreeByArea = (treeNodes: TreeNode[], area: string): TreeNode[] => {
    if (area === 'all') return treeNodes;
    const areaKey = normalizeArea(area);
    const traverse = (node: TreeNode, parentMatch: boolean): TreeNode | null => {
      const mine = normalizeArea(node.area) === areaKey;
      const childHas = node.children?.some((c) => {
        const check = (n: TreeNode): boolean => normalizeArea(n.area) === areaKey || (n.children?.some(check) ?? false);
        return check(c);
      }) ?? false;
      if (mine || childHas || parentMatch) {
        return {
          ...node,
          children: node.children?.map((c) => traverse(c, mine || parentMatch)).filter(Boolean) as TreeNode[] ?? [],
        };
      }
      return null;
    };
    return treeNodes.map((n) => traverse(n, false)).filter(Boolean) as TreeNode[];
  };
  ...
  const fetchTopology = useCallback(async () => {
    ...
    try {
      ...
      if (selectedArea !== 'all') params.append('area', selectedArea.trim());

      const baseUrl =
        viewMode === 'subtree' && locationId
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 3, gap: 3 }}>
            {(['subtree', 'tree'] as const).map((mode) => (
              ...
              >
                {mode === 'subtree' ? `Location${locationId ? ` ${locationId}` : ''}` : 'Full Tree'}
              </button>
            ))}
          </div>
