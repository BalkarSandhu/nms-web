...
const TopologyEditor = () => {
  ...
  const filterTreeByArea = (treeNodes: TreeNode[], area: string): TreeNode[] => {
    ...
    const filterNode = (node: TreeNode): TreeNode | null => {
      // Check if this node's area matches the filter
      ...
      // 2. It has children that match (to preserve the hierarchy under the matching area)
      if (matchesArea) {
        return {
          ...node,
          children: filteredChildren,  // Keep filtered children
        };
      } else if (filteredChildren.length > 0) {
        // If this node doesn't match but has children that do, include it to preserve hierarchy
        return {
          ...node,
  ...
  const fetchTopology = useCallback(async () => {
    try {
      ...
      if (viewMode === 'subtree' && locationId) {
        ...
        // Full tree mode: use /locations/tree endpoint (already returns tree structure with area field)
