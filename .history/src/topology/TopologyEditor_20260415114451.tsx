...
const getAuthHeaders = (): HeadersInit => {
  // Replace with your actual auth logic (e.g., from Redux store or localStorage)
...
const TopologyEditor = () => {
  ...
  // New state for view mode and location ID
  const [viewMode, setViewMode] = useState<'tree' | 'subtree'>('subtree');
  const [locationId, setLocationId] = useState<number>(36); // Default to CCWO COMMUNITY CENTER

  // API endpoint from environment variable - FIXED for Vite
  const API_BASE = import.meta.env.VITE_NMS_HOST || 'http://localhost:8080';

  // Fetch topology data from live API
  const fetchTopology = useCallback(async () => {
    try {
      ...
      if (viewMode === 'subtree' && locationId) {
        url = `${API_BASE}/locations/${locationId}/subtree`;
      } else {
        url = `${API_BASE}/locations/tree`;
      }

      console.log('Fetching topology from:', url); // Debug log
      ...
      // Handle different response structures (API might return { tree: [...] } or direct array)
      const treeData = data.tree || data;
      
      // Ensure treeData is an array
      const treeArray = Array.isArray(treeData) ? treeData : [treeData];
      
      const processed = processTreeData(treeArray);
  ...
  }, [API_BASE, viewMode, locationId, selectedArea]);
  ...
  // Update graph when area filter changes
  useEffect(() => {
    if (rawTreeData.length === 0) return;
  ...
  /* ─── Filtered nodes/edges for search/status ─── */
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {(['subtree', 'tree'] as const).map((mode) => (
              <button
                ...
                style={{
                  ...
                  border: viewMode === mode ? '1px solid rgba(16,185,129,0.3)' : '1px solid transparent',
                  borderRadius: 6,
                  cursor: 'pointer',
                  color: viewMode === mode ? '#6ee7b7' : '#64748b',
                  fontSize: 12,
                  fontWeight: 600,
                  transition: 'all 0.2s',
                }}
          ...
          {/* Location ID input (only visible in subtree mode) */}
          {viewMode === 'subtree' && (
            <input
              type="number"
              value={locationId}
      ...
      >
        ...
        <span>
          {autoRefresh ? '● Live · ' : ''}
          Mode: {viewMode === 'subtree' ? `Subtree (ID: ${locationId})` : 'Full Tree'} · 
