...
const TopologyEditor = () => {
  ...
  // Fix 3 — also trim in extractAreas to stay consistent
const extractAreas = (treeNodes: TreeNode[]): string[] => {
  const areaMap = new Map<string, string>();
  const traverse = (node: TreeNode) => {
    const value = node.area?.trim();  // trim here too
  ...
  // Fix 1 — add a fallback and debug log to catch silent mismatches
const filterTreeByArea = (treeNodes: TreeNode[], targetArea: string): TreeNode[] => {
  ...
  const traverse = (node: TreeNode): TreeNode | null => {
    const nodeKey = normalizeArea(node.area);  // already correct
  ...
  return treeNodes.map(traverse).filter((n): n is TreeNode => n !== null);
};

  /* ─── Convert Location to TreeNode ─── */
  // Fix 2 — normalize area values when extracting, and trim the API data
const processTreeData = (locs: Location[], level = 0): TreeNode[] =>
  locs.map((node) => ({
    ...node,
    area: node.area?.trim() ?? '',   // ← ADD THIS: normalize at source
