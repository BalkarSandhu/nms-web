...
const TopologyEditor = () => {
  ...
  // FIXED: Better area filtering that preserves tree structure
  const filterTreeByArea = (treeNodes: TreeNode[], targetArea: string): TreeNode[] => {
    if (targetArea === 'all') return treeNodes;
    
    const areaKey = normalizeArea(targetArea);
    
    const traverse = (node: TreeNode): TreeNode | null => {
      const nodeMatchesArea = normalizeArea(node.area) === areaKey;
      ...
      // Include node if:
      // 1. It matches the area, OR
      // 2. It has children that match the area (even if parent doesn't match)
  ...
  const fetchTopology = useCallback(async () => {
    ...
    try {
      ...
      // FIXED: Use consistent endpoint (don't change based on viewMode)
      ...
      // Store all tree data for filtering
      setAllTreeData(processed);
      setAvailableAreas(extractAreas(processed));

      // Apply area filter AFTER loading
  ...
  // FIXED: Only refetch when necessary (not on every area change)
  useEffect(() => { 
    fetchTopology(); 
  }, []);

  // FIXED: Update graph when area changes (without refetching API)
  useEffect(() => {
    ...
    >
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          >
            <option value="all" style={{ background: '#0f172a', color: '#e2e8f0' }}>All Areas</option>
            {availableAreas.map((a) => (
              <option key={a} value={a} style={{ background: '#0f172a', color: '#e2e8f0' }}>
                {a}
              </option>
            ))}
          </select>
          ...
          {/* View mode - REMOVED SUBTREE MODE */}
          <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: 3, gap: 3 }}>
            <button
              style={{ padding: '5px 12px', background: 'rgba(96,165,250,0.18)', border: '1px solid rgba(96,165,250,0.35)', borderRadius: 6, cursor: 'default', color: '#93c5fd', fontSize: 12, fontWeight: 600 }}
            >
              Full Tree
            </button>
          </div>
