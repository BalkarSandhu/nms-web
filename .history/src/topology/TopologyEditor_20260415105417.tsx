...
const TopologyEditor = () => {
  ...
  const extractAreas = (treeNodes: TreeNode[]): string[] => {
    ...
    const traverse = (node: TreeNode) => {
      if (node.area && node.area.trim()) {
        areaSet.add(node.area.trim());
      }
      node.children?.forEach(traverse);
    };
    treeNodes.forEach(traverse);
    return Array.from(areaSet).sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  };
  ...
  const filterTreeByArea = (treeNodes: TreeNode[], targetArea: string): TreeNode[] => {
    ...
    const traverse = (node: TreeNode): TreeNode | null => {
      // Check if this node's area matches the target area
      const nodeMatchesArea = node.area?.trim() === targetArea.trim();
      ...
      // 1. Its area matches the target area, OR
      // 2. It has children that belong to the target area
