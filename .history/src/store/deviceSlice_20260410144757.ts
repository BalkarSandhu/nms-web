...
// Helper to extract devices from response
const extractDevices = (response: any): readDeviceType[] => {
...
export const fetchDeviceTypes = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const deviceTypes = data.device_types || data.data || [];
...
export const fetchDeviceStatistics = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const data = await response.json();
      // Handle both old and new formats - new format might wrap in data
