...
// Helper to extract locations from response
...
const extractPagination = (response: any, page: number, perPage: number): PaginationMeta => {
  ...
  async (_, { rejectWithValue }) => {
    try {
      const queryParams = new URLSearchParams({
        page_size: '2000', // Fetch up to 2000 locations in single request
      });
...
export const fetchLocationsforMap = createAsyncThunk(
  'locations/fetchForMap',
  async (_, { rejectWithValue }) => {
    try {
      const url = buildUrlWithWorkerId(`${import.meta.env.VITE_NMS_HOST}/locations?fetch_all=true`);
      const response = await fetch(url, {
        headers: getAuthHeaders(),
      });
      if (response.status === 401) {
...
export const fetchLocationsPaginated = createAsyncThunk(
  ...
  async (params: { page: number; perPage: number }, { rejectWithValue }) => {
    try {
      ...
      const pagination = extractPagination(data, params.page, params.perPage);
...
export const fetchLocationTypes = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // ✅ API returns a plain array directly — handle both array and wrapped response
      if (Array.isArray(data)) {
        return data as LocationType[];
      }
      return extractLocations(data) as unknown as LocationType[];
    } catch (error) {
      return rejectWithValue((error as Error).message);
    }
  }
...
const locationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchLocationsforMap.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchLocationsforMap.fulfilled, (state, action) => {
        state.loading = false;
        state.locations = action.payload;
        state.lastFetched = Date.now();
      })
      .addCase(fetchLocationsforMap.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload as string;
      })
