...
export async function generateMultipleDevicesReport(
  ...
  // Top 5 most reliable / 5 least reliable — rendered as two separate side-by-side tables
  ...
  autoTable(doc, {
    startY: splitStartY,
    margin: { left: 40, right: 312 },
    head: [["#", "Most reliable", "Uptime"]],
    body: top.map((t, i) => [
      String(i + 1),
      t.device.display || t.device.hostname || "",
      fmtPct(t.agg.uptimePct),
    ]),
    headStyles: { fillColor: [15, 23, 42], textColor: 226 },
    styles:     { fontSize: 9, cellPadding: 4 },
  });
  ...
  autoTable(doc, {
    startY: splitStartY,
    margin: { left: 312, right: 40 },
    head: [["#", "Least reliable", "Uptime"]],
    body: bottom.map((b, i) => [
      String(i + 1),
      b.device.display || b.device.hostname || "",
      fmtPct(b.agg.uptimePct),
    ]),
    headStyles: { fillColor: [15, 23, 42], textColor: 226 },
    styles:     { fontSize: 9, cellPadding: 4 },
  });
...
export async function generateAreaReport(
  ...
  autoTable(doc, {
    startY: y,
    margin: { left: 40, right: 40 },
    head: [["Location", "Devices", "Avg Uptime", "Avg Latency", "Outages"]],
    body: locRows.map((r) => [
      r.locationName,
  ...
  // Top/bottom — rendered as two separate side-by-side tables
...
function drawAvailabilityChart(doc: jsPDF, y: number, W: number, history: HistoryEntry[], range: RangeKey): number {
  // Layout — chart area has padding for axis labels on the left and bottom.
  const h         = 140;                // total height including axis labels
  const leftPad   = 38;                 // room for Y-axis numbers
  const bottomPad = 26;                 // room for X-axis ticks + axis title
  const topPad    = 6;
  y = ensureSpace(doc, y, h + 20);

  const x0      = 40;
  const chartW  = W - 80;
  const innerX  = x0 + leftPad;
  const innerY  = y + topPad;
  const innerW  = chartW - leftPad - 4;
  ...
  if (history.length === 0) {
    ...
    doc.text("No history available", innerX + innerW / 2, innerY + innerH / 2, { align: "center" });
    return y + h + 12;
  }
  ...
  // Y-axis is "devices online" / "offline" / "no data", not raw probe counts.
  ...
  // ── Y-axis: pick a nice tick count and draw gridlines + numeric labels ──
  // Step is rounded so we get whole numbers on the axis (e.g. 0, 5, 10, 15…).
  function niceStep(maxVal: number, targetTicks: number): number {
    ...
    return Math.max(1, stepNorm * mag);
  }
  const yStep   = niceStep(maxY, 4);
  const yTopVal = Math.ceil(maxY / yStep) * yStep;

  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  for (let v = 0; v <= yTopVal; v += yStep) {
    const yPos = innerY + innerH - (v / yTopVal) * innerH;
    // gridline
    doc.setDrawColor(235, 240, 248);
    doc.line(innerX, yPos, innerX + innerW, yPos);
    // tick label
    doc.text(String(v), innerX - 3, yPos + 2.5, { align: "right" });
  }

  // Y-axis title
  doc.setFontSize(8);
  doc.setTextColor(71, 85, 105);
  // Rotate "Devices" 90deg, positioned just left of the plot area
  doc.text("Devices", x0 + 4, innerY + innerH / 2, { angle: 90, align: "center" });

  // ── Bars — drawn inside the plot area only ──────────────────────────────
  const barSlot = innerW / series.length;
  const barW    = Math.max(0.5, barSlot - 1);
  series.forEach((s, i) => {
    const x = innerX + i * barSlot + 0.5;
    const onlineH  = (s.online  / yTopVal) * innerH;
    const offlineH = (s.offline / yTopVal) * innerH;
    const noDataH  = (s.noData  / yTopVal) * innerH;

    if (onlineH > 0) {
      doc.setFillColor(16, 185, 129);
      doc.rect(x, innerY + innerH - onlineH, barW, onlineH, "F");
    }
    if (offlineH > 0) {
      doc.setFillColor(239, 68, 68);
      doc.rect(x, innerY + innerH - onlineH - offlineH, barW, offlineH, "F");
    }
    if (noDataH > 0) {
      doc.setFillColor(100, 116, 139);
      doc.rect(x, innerY + innerH - onlineH - offlineH - noDataH, barW, noDataH, "F");
    }
  });

  // ── X-axis ticks + labels — evenly spaced, with width-aware count ───────
  // Pick a label count that leaves room for each label (avoid overlap).
  const labelWidthPx = granularity === "daily" ? 56 : granularity === "hourly" ? 70 : 50;
  const maxLabels    = Math.max(2, Math.min(8, Math.floor(innerW / labelWidthPx)));
  const tickCount    = Math.min(series.length, maxLabels);
  const tickIndices: number[] = [];
  if (tickCount === 1) {
    tickIndices.push(0);
  } else {
    for (let k = 0; k < tickCount; k++) {
      const idx = Math.round((k * (series.length - 1)) / (tickCount - 1));
  ...
  for (const i of tickIndices) {
    const xPos = innerX + i * barSlot + barSlot / 2;
    // tick mark
    doc.line(xPos, innerY + innerH, xPos, innerY + innerH + 3);
    const align: "left" | "center" | "right" =
      i === 0 ? "left"
      : i === series.length - 1 ? "right"
      : "center";
    doc.text(
      formatBucketLabel(series[i].bucketStart, granularity),
      align === "left"  ? xPos - 1
      : align === "right" ? xPos + 1
      : xPos,
      innerY + innerH + 9,
  ...
  doc.setFontSize(8);
  doc.setTextColor(71, 85, 105);
  doc.text("Time", innerX + innerW / 2, innerY + innerH + 20, { align: "center" });

  // ── Legend — top right ──────────────────────────────────────────────────
  const legendY = y - 2;
  doc.setFontSize(8);
  doc.setFillColor(16, 185, 129);
  doc.rect(x0 + chartW - 160, legendY, 8, 8, "F");
  doc.setTextColor(71, 85, 105);
  doc.text("Online",  x0 + chartW - 148, legendY + 7);
  doc.setFillColor(239, 68, 68);
  doc.rect(x0 + chartW - 110, legendY, 8, 8, "F");
  doc.text("Offline", x0 + chartW - 98,  legendY + 7);
  doc.setFillColor(100, 116, 139);
  doc.rect(x0 + chartW - 60,  legendY, 8, 8, "F");
  doc.text("No data", x0 + chartW - 48,  legendY + 7);
...
function drawLatencyHistogram(doc: jsPDF, y: number, W: number, history: HistoryEntry[], agg: Aggregate): number {
  ...
  // Bin — manual min/max to avoid Math.min(...values) / Math.max(...values) stack
  // overflow when `values` is large (e.g. 1-month raw history).
  let min0 = Infinity;
  let max0 = 1;
...
// reduce-based to avoid Math.max(...arr) / Math.min(...arr) overflowing the call stack
// on large arrays (V8 caps function args around 65k–125k).
