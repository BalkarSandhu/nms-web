...
export async function generateAreaReport(
  ...
  y = drawKpiGrid(doc, y, W, [
    ...
    { label: "Total Checks",      value: fmt(overall.totalChecks) },
    { label: "Outage events",     value: String(overall.incidentCount),             tint: overall.incidentCount > 0 ? [239,68,68] : [16,185,129] },
...
function emptyAggregate(): Aggregate {
  return {
    ...
    avgLatency: 0, maxLatency: 0, minLatency: 0, p95Latency: 0,
...
function combineAggregates(aggs: Aggregate[]): Aggregate {
  ...
  return {
    ...
    p95Latency:   max(ok.map((a) => a.p95Latency)),
...
function kpiTilesForAggregate(a: Aggregate, compact = false): Kpi[] {
  const base: Kpi[] = [
    ...
    { label: "P95 Latency",     value: fmtMs(a.p95Latency) },
...
function drawAvailabilityChart(doc: jsPDF, y: number, W: number, history: HistoryEntry[], range: RangeKey): number {
  ...
  const buckets = new Map<number, { online: number; offline: number; total: number }>();
  for (let t = start.getTime(); t < end.getTime(); t += bucketMs) {
    buckets.set(Math.floor(t / bucketMs) * bucketMs, { online: 0, offline: 0, total: 0 });
  }
  for (const e of history) {
    const t = new Date(e.timestamp).getTime();
    const k = Math.floor(t / bucketMs) * bucketMs;
    let b = buckets.get(k);
    if (!b) { b = { online: 0, offline: 0, total: 0 }; buckets.set(k, b); }
    if (e.is_reachable) b.online += 1; else b.offline += 1;
    b.total += 1;
  }
  const ordered = [...buckets.entries()].sort((a, b) => a[0] - b[0]);
  const n = ordered.length;
  if (n === 0) return y + h + 12;

  const maxTotal = Math.max(1, ...ordered.map(([, b]) => b.total));
  const barW = Math.max(1, (chartW - 8) / n - 1);

  ordered.forEach(([, b], i) => {
    const x = x0 + 4 + i * (barW + 1);
    const totalH = (b.total / maxTotal) * (h - 16);
    const offH = b.total ? (b.offline / b.total) * totalH : 0;
    const onH  = totalH - offH;

    // Online (bottom)
    doc.setFillColor(16, 185, 129);
    doc.rect(x, y + h - 8 - onH, barW, onH, "F");
    // Offline (top)
    if (offH > 0) {
      doc.setFillColor(239, 68, 68);
      doc.rect(x, y + h - 8 - onH - offH, barW, offH, "F");
    }
  });
  ...
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  doc.text(formatBucketLabel(ordered[0][0], granularity), x0 + 4, y + h + 10);
  doc.text(formatBucketLabel(ordered[ordered.length - 1][0], granularity), x0 + chartW - 60, y + h + 10);

  // Legend
  doc.setFontSize(8);
  doc.setFillColor(16, 185, 129);
  doc.rect(x0 + chartW - 110, y - 12, 8, 8, "F");
  doc.text("Online", x0 + chartW - 98, y - 5);
  doc.setFillColor(239, 68, 68);
  doc.rect(x0 + chartW - 60, y - 12, 8, 8, "F");
  doc.text("Offline", x0 + chartW - 48, y - 5);
...
function drawLatencyHistogram(doc: jsPDF, y: number, W: number, history: HistoryEntry[], agg: Aggregate): number {
  ...
  doc.text(`avg ${fmtMs(agg.avgLatency)}    max ${fmtMs(agg.maxLatency)}    p95 ${fmtMs(agg.p95Latency)}`, x0 + 4, y - 6);
...
function deviceNarrative(device: DeviceMeta, agg: Aggregate, range: RangeKey): string {
  ...
  const lossNote = agg.avgPacketLoss > 1
    ...
    : ``;
  return `Over the ${period}, ${name} was ${reliability} with ${fmtPct(agg.uptimePct)} uptime across ${agg.totalChecks} checks. Latency was ${latencyWord} at an average of ${fmtMs(agg.avgLatency)} (P95 ${fmtMs(agg.p95Latency)}).${lossNote}`;
}
