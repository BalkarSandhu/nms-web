...
export default function Dashboard({ isButtonClicked }: DashboardProps) {
	...
	const activeLocations = Array.isArray(reduxLocations) ? reduxLocations : reduxLocations?.data || [];
	...
	const handleDeviceStatusClick = (status: 'unknown' | 'online' | 'offline') => {
		if (status === 'online') {
			navigate(`/devices?is_reachable=true`);
		} else if (status === 'offline') {
			navigate(`/devices?is_reachable=false`);
		} else {
			navigate(`/devices`);
		}
	};
