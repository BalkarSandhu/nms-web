    ...
    return (
        <BaseCard title={title} menuGroups={menuGroups} className={className}>
            <div className="flex flex-col w-full h-full">
                ...
                {/* Table Rows */}
                <div className="flex flex-col gap-2 mt-2 overflow-y-auto flex-1" style={{ maxHeight: '180px' }}>
                    {data.map((row) => (
                        <button
                            key={row.id}
                        ...
                        >
                            ...
                            <span className="text-(--contrast) text-sm text-right truncate font-small">{row.col2}</span>
                        </button>
                    ))}
