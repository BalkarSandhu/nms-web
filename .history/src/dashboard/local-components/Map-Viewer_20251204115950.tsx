...
export const MapViewer = ({
  ...
  // Handle hover timeout for topology node panel
  useEffect(() => {
    if (!isHoveringNode && !isHoveringPanel) {
      // Set timeout to close the panel
      hoverTimeoutRef.current = setTimeout(() => {
        setHoveredNode(null);
      }, 200); // Small delay to allow smooth transition
    } else {
      // Clear timeout if hovering
  ...
  }, [isHoveringNode, isHoveringPanel]);
          ...
          <div className="flex items-start justify-between mb-3 pb-3 border-b border-white/10">
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                ></div>
                <h3 className="text-white font-bold text-lg">Site Info.</h3>
              </div>
              <p className="text-gray-400 text-xs">
        ...
        {showPoints && enableTopology && topologyNodes.map(node => (
          ...
          >
            <div 
              ...
              onMouseLeave={() => {
                // Don't clear immediately - let the panel handle it
              }}
              onClick={() => {
                setHoveredPoint(null);
                setHoveredNode(node);
