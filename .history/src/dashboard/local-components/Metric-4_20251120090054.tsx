...
export default function Metric4({ 
    ...
    return (
        <BaseCard title={title} menuGroups={resolvedMenuGroups} className={className}>
            <div className="flex flex-col gap-2.5 h-full py-1">
                {/* Total Display */}
                <div className="text-center border-b border-(--contrast)/10 pb-2.5">
                    ...
                    {data.slice(0, 4).map((item, index) => {
                        ...
                        return (
                            <button
                                ...
                                className={`${color.bg} border ${color.border} rounded-lg p-2.5 transition-all hover:scale-[1.03] active:scale-[0.98]`}
                            >
                                <div className="flex flex-col items-start gap-2 h-full justify-between">
                                    ...
                                    <div className="w-full">
                                        <div className="text-(--contrast) font-bold text-2xl leading-none">{item.count}</div>
                                        <div className={`${color.text} text-[10px] font-semibold truncate mt-1`}>
                                            {item.type}
                                        </div>
                                        <div className={`${color.text} text-[10px] mt-0.5 opacity-70`}>
                                            {percentage}%
                                        </div>
                                    </div>
                                </div>
                ...
                {/* Show more indicator */}
                {data.length > 4 && (
                    <div className="text-center text-(--contrast)/40 text-[10px] font-medium -mt-1">
