...
export default function MetricGeneral({
    ...
    // Memoize sorted data by value with deduplication
    const sortedData = useMemo(() => {
        if (!data.length) return [];
        
        // Deduplicate by label: combine values for items with the same label
        const deduplicatedMap = new Map<string, MetricItem>();
        
        data.forEach((item, idx) => {
            const label = item.label?.trim(); // Trim whitespace to catch slight variations
            if (!label) return; // Skip items without labels
            
            if (deduplicatedMap.has(label)) {
                // Combine values for duplicate labels
                const existing = deduplicatedMap.get(label)!;
                deduplicatedMap.set(label, {
                    ...existing,
                    value: existing.value + item.value
                });
                console.log(`Merged duplicate: ${label}, new value: ${existing.value + item.value}`);
            } else {
                deduplicatedMap.set(label, { ...item, label });
            }
        });
        
        // Convert back to array and sort by value
        const result = Array.from(deduplicatedMap.values()).sort((a, b) => b.value - a.value);
        console.log(`Original data length: ${data.length}, Deduplicated length: ${result.length}`);
    ...
    // Donut Chart Component with Hover Tooltip
    const DonutChart = () => {
        const displayData = sortedData.slice(0, maxItems);
        let cumulativePercent = 0;

        console.log(`Rendering donut with ${displayData.length} segments:`, displayData.map(d => d.label));
        ...
        return (
            <div className="flex items-center gap-6 h-full py-3 px-4">
                ...
                <div className="relative flex-shrink-0" style={{ width: '120px', height: '120px' }}>
                        ...
                        {displayData.map((item, index) => {
                            ...
                            return (
                                <circle
                                    ...
                                    style={{
                                        filter: hoveredItem === item.label 
                                            ? 'drop-shadow(0 0 12px rgba(0,0,0,0.5))' 
                                            : 'drop-shadow(0 0 8px rgba(0,0,0,0.3))',
                                        strokeWidth: hoveredItem === item.label ? '18' : '16',
                                    }}
                                />
                            );
                        })}
                    ...
                    {/* Center Display - shows hovered item count or total */}
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        {hoveredItem ? (
                            <>
                                <span className="text-3xl font-bold text-(--contrast) transition-all duration-200">
                                    {displayData.find(item => item.label === hoveredItem)?.value}
                                </span>
                            </>
                        ) : (
                            <>
                    ...
                    {displayData.map((item, index) => {
                        ...
                        return (
                            <button
                                ...
                                onMouseEnter={() => setHoveredItem(item.label)}
                                onMouseLeave={() => setHoveredItem(null)}
                                className={`flex items-center gap-2 px-2 py-1 rounded-md transition-all group ${
                                    isHovered ? 'bg-(--contrast)/10' : 'hover:bg-(--contrast)/5'
                                }`}
                            >
                                <div 
                                    className="w-3 h-3 rounded-full flex-shrink-0 ring-2 ring-offset-1 ring-offset-(--dark) transition-all"
                                    style={{ 
                                ...
                                <div className="flex items-center gap-1.5 flex-1 min-w-0">
                                    ...
                                    <span className="text-(--contrast) text-xs font-medium truncate">
                                        {item.label}
                                    </span>
                                </div>
