...
// Function to group devices by coordinates
...
export const MapViewer = ({
  ...
  const locationConnectionsGeoJSON = useMemo(() => {
    ...
    const locationGroups = new Map<string, MapDataPoint[]>();
    filteredData.forEach(device => {
      if (device.location) {
        if (!locationGroups.has(device.location)) {
          locationGroups.set(device.location, []);
        }
        locationGroups.get(device.location)!.push(device);
      }
    });

    // Create connections between all devices in the same location group
    locationGroups.forEach((devices, locationName) => {
      if (devices.length > 1) {
        // Remove devices that are part of topology groups from location connections
        // We only want to connect devices that are at DIFFERENT coordinates
        const devicesAtDifferentCoords = devices.filter(device => {
          // Check if this device is part of a topology group
          const isInTopology = topologyNodes.some(node => 
            node.devices.some(d => d.id === device.id)
          );
          return !isInTopology;
        });
        
        // Also include one representative from each topology node in this location
        const topologyRepsInLocation = new Map<string, MapDataPoint>();
        topologyNodes.forEach(node => {
          // Check if any device in this topology node belongs to this location
          const deviceInLocation = node.devices.find(d => d.location === locationName);
          if (deviceInLocation) {
            // Use the center coordinates of the topology node
            topologyRepsInLocation.set(node.id, {
              ...node.devices[0],
        ...
        // Combine standalone devices with topology node representatives
        const allConnectionPoints = [...devicesAtDifferentCoords, ...Array.from(topologyRepsInLocation.values())];
        ...
        for (let i = 0; i < allConnectionPoints.length; i++) {
          for (let j = i + 1; j < allConnectionPoints.length; j++) {
            features.push({
              ...
              geometry: {
                type: 'LineString' as const,
                coordinates: [allConnectionPoints[i].coordinates, allConnectionPoints[j].coordinates]
              },
              properties: {
                id: `location-${locationName}-${allConnectionPoints[i].id}-${allConnectionPoints[j].id}`,
