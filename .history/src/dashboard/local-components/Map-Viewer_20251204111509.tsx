...
export const MapViewer = ({
  ...
  const locationConnectionsGeoJSON = useMemo(() => {
    ...
    locationGroups.forEach((devices, locationName) => {
      if (devices.length > 1) {
        // Create mesh connections (connect each device to every other device in the group)
        for (let i = 0; i < devices.length; i++) {
          for (let j = i + 1; j < devices.length; j++) {
            features.push({
              ...
              geometry: {
                type: 'LineString' as const,
                coordinates: [devices[i].coordinates, devices[j].coordinates]
              },
              properties: {
                id: `location-${locationName}-${devices[i].id}-${devices[j].id}`,
