...
export default function MetricGeneral({
    ...
    const grouped = data.reduce<Record<string, MetricItem>>((acc, item) => {
        ...
        return (
            <div className="flex items-center gap-6 h-full py-3 px-4">
                ...
                <div className="relative flex-shrink-0" style={{ width: '120px', height: '120px' }}>
                    ...
                    {/* Center Display - shows hovered item or total */}
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                        {hoveredItem ? (
                            <>
                                <span className="text-xl font-bold text-(--contrast) transition-all duration-200">
                                    {displayData.find(item => item.label === hoveredItem)?.value}
                                </span>
                                <span className="text-[9px] text-(--contrast)/80 font-medium text-center px-2 line-clamp-2">
                                    {hoveredItem}
                                </span>
                            </>
                        ) : (
