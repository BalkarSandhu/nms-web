...
export const MapViewer = ({
  ...
  // This creates polygons by connecting the outer points of topology groups
  const locationConnectionsGeoJSON = useMemo(() => {
    const features: any[] = [];
    
    // Group topology nodes and single devices by location name
    const locationGroups = new Map<string, Array<{ coordinates: [number, number]; id: string | number }>>();
    
    // Add topology nodes (use their center coordinates)
    topologyNodes.forEach(node => {
      const location = node.devices[0].location;
      if (location) {
    ...
    // Create connections between all points in the same location group
    locationGroups.forEach((points, locationName) => {
      if (points.length > 1) {
        // Create mesh connections (connect each point to every other point)
