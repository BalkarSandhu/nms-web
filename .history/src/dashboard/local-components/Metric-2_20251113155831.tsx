    ...
    return (
        <BaseCard title={title} menuGroups={menuGroups} className={className}>
            <div className="flex flex-col w-full h-full">
                ...
                {/* Table Rows */}
                <div className="flex flex-col gap-1 mt- 0.5overflow-y-auto">
                    {displayedRows.map((row) => (
                        <button
                            key={row.id}
                            onClick={() => handleRowClick(row.link)}
                            className="grid grid-cols-2 gap-2 py-px px-2 rounded-[5px] 
                                     hover:bg-(--contrast)/10 transition-colors text-left
                                     border border-transparent hover:border-(--contrast)/20"
                        >
                            <span className="text-(--contrast) text-[11px] truncate">{row.col1}</span>
                            <span className="text-(--contrast) text-[11px] text-right truncate">{row.col2}</span>
                        </button>
                    ))}
