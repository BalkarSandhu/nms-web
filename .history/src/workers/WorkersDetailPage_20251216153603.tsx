...
export default function WorkerDetailPage() {
  ...
  useEffect(() => {
    const dataPoints: MapDataPoint[] = locationDevices.map(device => ({
      ...
      value: device.is_reachable ? 1 : 0,
      category: device.is_reachable ? 'green' : 'red',
      popupData: {
        indicatorColour: device.is_reachable ? 'green' : 'red',
        headerLeft: { field: 'Device', value: device.hostname },
        headerRight: { field: 'Status', value: device.is_reachable ? 'true' : 'false' },
    ...
    if (location && location.latitude && location.longitude && locationDevices.length > 0) {
      locationDevices.forEach((device, index) => {
        connections.push({
          ...
          from: 'location-main',
          to: device.id,
          color: device.is_reachable ? '#4CB944' : '#D52941',
          width: device.is_reachable ? 2 : 1,
                      ...
                      {locationDevices.map((device) => (
                        <TableRow key={device.id}>
                          ...
                          <TableCell>
                            ...
                            >
                              {device.is_reachable ? "true" : "false"}
                            </span>
                          </TableCell>
                        </TableRow>
                      ))}
