...
export default function AddDeviceForm() {
  ...
  const { refreshVersion, triggerRefresh } = useRefresh();
  ...
  const areaOptions = useMemo(() => {
    ...
    const options = [...set].sort((a, b) => a.localeCompare(b));
    console.log("[AddDeviceForm] Area options:", options, "from locations:", allLocations);
    return options;
  }, [allLocations]);
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        console.log("[AddDeviceForm] Fetching locations...");
        const locations = await getLocations();
        console.log("[AddDeviceForm] Locations loaded:", locations);
        setAllLocations(locations);
      } catch (error) {
        console.error("[AddDeviceForm] Error fetching locations:", error);
        setAllLocations([]);
      }
    };
    fetchLocations();
