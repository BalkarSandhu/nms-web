    ...
    return (
        <div className="bg-gray-50 rounded-lg p-3 border border-gray-200 hover:border-blue-300 transition-colors">
            <div className="flex items-start justify-between mb-2">
                ...
                <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${device.is_reachable
            ...
            <div className="space-y-1">
                ...
                {device.last_check && (
                    <div className="flex justify-between text-xs">
                        ...
                        <span className="text-gray-900">{new Date(device.last_check).toLocaleTimeString()}</span>
                    </div>
                )}
                {device.last_check_time_taken !== undefined && (
                    <div className="flex justify-between text-xs">
                        <span className="text-gray-500">Response:</span>
                        <span className="text-gray-900">{device.last_check_time_taken.toFixed(2)}ms</span>
                    </div>
                )}
            </div>
        </div>
