...
import { ArrowLeft, MapPin, Activity, Clock, Server, X, FileText } from 'lucide-react';
  ...
  useEffect(() => {
    if (locationId && location) {
      ...
      generatedLogs.push({
        ...
        level: location.status === 'online' ? 'success' : 'error',
        message: `Location status: ${location.status}`,
      });
      ...
      locationDevices.forEach(device => {
        generatedLogs.push({
          ...
          level: device.status ? 'success' : 'error',
          message: device.status_reason || (device.status ? 'Device online' : 'Device offline'),
    ...
    <div className="min-h-screen bg-gray-100 p-2">
      ...
      {showMapModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-2xl w-[100vh] h-[90vh] flex flex-col">
            {/* Modal Header */}
