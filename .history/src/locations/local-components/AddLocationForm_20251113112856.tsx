...
export const AddLocationForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!area || !lat || !lng || !locationType || !name ||!project || !statusI || !statusReason || !workerId) {
      setStatus({
        message: "Area, Name, Location Type, and Area are required.",
        type: "error",
      });
      return;
    }
    ...
    try {
      await addLocation({area, lat: Number(lat), lng: Number(lng), locationType, name, parentLocation, project, statusI, statusReason, workerId });
