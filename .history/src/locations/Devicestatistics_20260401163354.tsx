...
const MetricBar = ({ title, value, unit }: { title: string; value: number; unit: string }) => {
  ...
  return (
    <div className="bg-slate-800 p-3 rounded-xl border border-slate-700 h-full">
      ...
      <div className="flex items-center justify-between mb-2">
        <span className="text-xl font-bold" style={{ color }}>{value.toFixed(2)}{displaySuffix}</span>
        <span className="text-xs text-slate-400">{gaugeScore.toFixed(0)}%</span>
      </div>
      <div className="h-2 w-full rounded-full bg-slate-900 overflow-hidden border border-slate-700">
        <div
          className="h-full rounded-full transition-all duration-300"
          style={{ width: `${gaugeScore}%`, backgroundColor: color }}
        />
      </div>
        ...
        {uptimeLoading ? (
          ...
          <div className="space-y-3">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
              <MetricBar title="Packet Loss" value={uptimeData.avg_packet_loss_percent} unit="%" />
              <MetricBar title="Avg Jitter (ms)" value={uptimeData.avg_jitter_ms} unit="ms" />
              <MetricBar title="Avg Latency (ms)" value={uptimeData.avg_latency_ms} unit="ms" />
            </div>
