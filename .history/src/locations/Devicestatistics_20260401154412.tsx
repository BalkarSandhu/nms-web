...
export default function DeviceStatistics({ devices = [], deviceId }: DeviceStatisticsProps) {
  ...
  if (deviceId && !devices.length) {
    return (
      <Card className="transition-all hover:shadow-lg shadow-sm border border-gray-200">
        <CardHeader>
          <div className="flex items-start justify-between flex-wrap gap-2">
            <CardTitle className="text-base font-semibold text-gray-900">Device Statistics</CardTitle>
        ...
        <CardContent>
          <div className={`rounded-lg p-4 transition-all ${
            uptimeData && uptimeData.uptime_pct >= 95 
              ? 'bg-green-50 border border-green-100' 
              : uptimeData && uptimeData.uptime_pct >= 80 
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                    <div className="text-xs uppercase tracking-wide text-gray-500 mb-1">Uptime</div>
                    <div className="text-2xl font-bold text-green-600">{uptimeData.uptime_pct.toFixed(1)}%</div>
                  </div>
                  <div className="bg-white rounded-lg p-3 border border-gray-200 shadow-sm">
                    <div className="text-xs uppercase tracking-wide text-gray-500 mb-1">Downtime</div>
                    <div className="text-2xl font-bold text-red-600">{(100 - uptimeData.uptime_pct).toFixed(1)}%</div>
                  </div>
                </div>
                  ...
                  <div className="h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div
                      className={`h-full transition-all duration-500 ${
                        uptimeData.uptime_pct >= 95 ? 'bg-green-600' : 
                        uptimeData.uptime_pct >= 80 ? 'bg-amber-500' : 'bg-red-500'
                      }`}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Online Checks</div>
                    <div className="text-lg font-semibold text-sky-700">{uptimeData.online_checks}</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Offline Checks</div>
                    <div className="text-lg font-semibold text-rose-700">{uptimeData.offline_checks}</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Avg Latency</div>
                    <div className="text-lg font-semibold text-amber-700">{uptimeData.avg_latency_ms.toFixed(2)} ms</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Max Latency</div>
                    <div className="text-lg font-semibold text-violet-700">{uptimeData.max_latency_ms.toFixed(2)} ms</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Avg Jitter</div>
                    <div className="text-lg font-semibold text-orange-600">{uptimeData.avg_jitter_ms.toFixed(2)} ms</div>
                  </div>
                  <div className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm">
                    <div className="text-xs text-gray-500">Packet Loss</div>
                    <div className="text-lg font-semibold text-pink-600">{uptimeData.avg_packet_loss_percent.toFixed(2)}%</div>
                  </div>
                </div>

                <div className="bg-white rounded-lg p-3 border border-gray-200 shadow-sm grid grid-cols-2 gap-2 text-xs text-gray-600">
                  <div className="flex justify-between"><span>Total checks</span><span className="text-gray-900 font-semibold">{uptimeData.total_checks}</span></div>
                  <div className="flex justify-between"><span>Power cuts</span><span className="text-gray-900 font-semibold">{uptimeData.power_cut_count}</span></div>
                  <div className="flex justify-between"><span>Power restored</span><span className="text-gray-900 font-semibold">{uptimeData.power_restored_count}</span></div>
                  <div className="flex justify-between"><span>Longest downtime</span><span className="text-gray-900 font-semibold">{Math.floor(uptimeData.longest_downtime_seconds / 3600)}h {Math.floor((uptimeData.longest_downtime_seconds % 3600) / 60)}m</span></div>
                </div>
              </div>
            ) : (
