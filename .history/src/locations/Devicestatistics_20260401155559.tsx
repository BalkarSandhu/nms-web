...
export default function DeviceStatistics({ devices = [], deviceId }: DeviceStatisticsProps) {
  ...
  if (deviceId && !devices.length) {
    return (
      <div className={`border rounded-lg p-3 transition-all hover:shadow-sm ${
        ...
        {uptimeLoading ? (
          ...
          <div className="flex items-center gap-2 py-4 px-3 bg-red-900 border border-red-700 rounded text-red-300 text-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div className="bg-gray-800 rounded-lg p-2 border border-gray-600">
                <div className="flex items-center gap-1 mb-1">
                  ...
                  <span className="text-xs text-gray-400 font-medium">Downtime</span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                <span className="text-xs text-gray-400">Online Checks</span>
                <p className="font-semibold text-blue-400">{uptimeData.online_checks}</p>
              </div>
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                <span className="text-xs text-gray-400">Offline Checks</span>
                <p className="font-semibold text-red-400">{uptimeData.offline_checks}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                ...
                <p className="font-semibold text-amber-400">{uptimeData.avg_latency_ms.toFixed(2)}ms</p>
              </div>
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                <span className="text-xs text-gray-400">Max Latency</span>
                <p className="font-semibold text-purple-400">{uptimeData.max_latency_ms.toFixed(2)}ms</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                <span className="text-xs text-gray-400">Avg Jitter</span>
                <p className="font-semibold text-orange-400">{uptimeData.avg_jitter_ms.toFixed(2)}ms</p>
              </div>
              <div className="bg-gray-800 rounded p-2 border border-gray-600">
                <span className="text-xs text-gray-400">Packet Loss</span>
                <p className="font-semibold text-pink-400">{uptimeData.avg_packet_loss_percent.toFixed(2)}%</p>
              </div>
            </div>
            ...
            <div className="text-xs text-gray-400 space-y-1 pt-2 border-t border-gray-600">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-white">{uptimeData.total_checks}</span>
              </div>
              <div className="flex justify-between">
                <span>Power Cuts:</span>
                <span className="font-semibold text-white">{uptimeData.power_cut_count}</span>
              </div>
              <div className="flex justify-between">
  ...
  const getUptimeBackgroundColor = (uptime: number): string => {
    if (uptime >= 95) return 'bg-green-900 border-green-700';
    if (uptime >= 80) return 'bg-yellow-900 border-yellow-700';
