          ...
          <div className="space-y-3">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
              {[
                {
                  title: 'Packet Loss',
                  value: uptimeData.avg_packet_loss_percent,
                  color: '#f43f5e',
                  thresholds: [uptimeData.avg_packet_loss_percent, Math.max(0, 100 - uptimeData.avg_packet_loss_percent)],
                },
                {
                  title: 'Avg Jitter (ms)',
                  value: uptimeData.avg_jitter_ms,
                  color: uptimeData.avg_jitter_ms <= 20 ? '#22c55e' : uptimeData.avg_jitter_ms <= 50 ? '#f59e0b' : '#ef4444',
                  thresholds: [Math.min(100, uptimeData.avg_jitter_ms), Math.max(0, 100 - Math.min(100, uptimeData.avg_jitter_ms))],
                },
                {
                  title: 'Avg Latency (ms)',
                  value: uptimeData.avg_latency_ms,
                  color: uptimeData.avg_latency_ms <= 80 ? '#22c55e' : uptimeData.avg_latency_ms <= 120 ? '#f59e0b' : '#ef4444',
                  thresholds: [Math.min(100, uptimeData.avg_latency_ms), Math.max(0, 100 - Math.min(100, uptimeData.avg_latency_ms))],
                },
              ].map((metric) => (
                <div key={metric.title} className="bg-slate-800 p-2 rounded-lg border border-slate-700">
                  <h4 className="text-xs font-semibold text-slate-300 mb-1">{metric.title}</h4>
                  <div className="h-24 w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={[{ name: metric.title, value: metric.thresholds[0] }, { name: 'Remaining', value: metric.thresholds[1] }]} innerRadius={24} outerRadius={36} dataKey="value" startAngle={90} endAngle={-270}>
                          <Cell key="good" fill={metric.color} />
                  ...
                  <div className="mt-1 text-center">
                    <span className="text-sm font-bold" style={{ color: metric.color }}>
                      {metric.value.toFixed(2)}{metric.title.includes('Latency') || metric.title.includes('Jitter') ? 'ms' : '%'}
                    </span>
                  </div>
                </div>
              ))}
