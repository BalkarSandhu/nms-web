...
export default function DowntimePage() {
  ...
  // For every device we fetch its history over `range` and calculate downtime
  // percentage. The cards then reflect the chosen timeline downtime stats.
  ...
  useEffect(() => {
    ...
    mapLimit(
      ...
      FETCH_CONCURRENCY,
      async (d: any) => {
        const history = await fetchDeviceHistory(d.id, start, end, granularity);
  ...
  // Per-area downtime stats derived from device downtime data.
  const areaCards = useMemo(
    () =>
      areasSorted.map((w) => {
        ...
        if (view === "locations") {
          // Calculate average downtime per location
          ...
          for (const d of inArea) {
            ...
            if (!byLoc.has(loc)) {
              byLoc.set(loc, { totalDowntime: 0, count: 0 });
            }
            const current = byLoc.get(loc)!;
            byLoc.set(loc, {
              totalDowntime: current.totalDowntime + downtime,
        ...
        for (const d of inArea) {
          const data = downtimeState.get(d.id);
          if (data) {
            totalDowntime += data.downtimePct;
            count++;
          }
        }
  ...
  return (
    <div className="min-h-screen bg-gray-900 p-4 md:p-6 fade-in">
      ...
      {selectedDevice ? (
        ...
        // 3b. Area cards — click one to see downtime details
