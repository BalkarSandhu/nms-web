...
import DowntimeModal, { type DowntimeRecord } from "@/downtime/DowntimeModal";
...
function DowntimeColorCell({ pct }: { pct: number }) {
  ...
  if (pct <= 5) color = "var(--status-online)";
  else if (pct <= 15) color = "var(--status-warning)";
  else color = "var(--status-offline)";
        ...
        <Table className="table-fixed w-full [&_td]:align-top [&_td]:whitespace-normal [&_td]:break-words [&_td]:px-3 [&_td]:py-2.5 [&_th]:px-3 [&_th]:py-3">
          <TableHeader className="sticky top-0 z-10" style={{ background: "rgba(15,23,42,0.92)" }}>
            <TableRow style={{ borderColor: "var(--border-soft)" }}>
              ...
              <TableHead className={`w-[16%] ${headClass}`} style={headStyle}>IP Address</TableHead>
              <TableHead className={`w-[15%] ${headClass}`} style={headStyle}>Area</TableHead>
              <TableHead className={`w-[15%] ${headClass}`} style={headStyle}>Location</TableHead>
              <TableHead className={`w-[12%] ${headClass}`} style={headStyle}>Type</TableHead>
              <TableHead className={`w-[10%] text-right ${headClass}`} style={headStyle}>Downtime %</TableHead>
              <TableHead className={`w-[7%] text-right ${headClass}`} style={headStyle}>Events</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {rows.length === 0 ? (
              <TableRow>
                <TableCell colSpan={8} className="text-center py-12" style={{ color: "var(--text-lo)" }}>
                  No devices in scope
                </TableCell>
              </TableRow>
            ) : (
      ...
      <Table className="table-fixed w-full [&_td]:align-top [&_td]:px-3 [&_td]:py-2.5 [&_th]:px-3 [&_th]:py-3">
        <TableHeader className="sticky top-0 z-10" style={{ background: "rgba(15,23,42,0.92)" }}>
          <TableRow style={{ borderColor: "var(--border-soft)" }}>
            ...
            <TableHead className={`w-[35%] ${headClass}`} style={headStyle}>Location Name</TableHead>
            <TableHead className={`w-[20%] ${headClass}`} style={headStyle}>Area</TableHead>
            <TableHead className={`w-[15%] text-center ${headClass}`} style={headStyle}>Device Count</TableHead>
            <TableHead className={`w-[22%] text-right ${headClass}`} style={headStyle}>Avg Downtime %</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {rows.length === 0 ? (
            <TableRow>
              <TableCell colSpan={5} className="text-center py-12" style={{ color: "var(--text-lo)" }}>
                No locations in scope
              </TableCell>
            </TableRow>
          ) : (
            rows.map((l, index) => (
              ...
              >
                <TableCell className="text-center text-sm tabular-nums" style={{ color: "var(--text-lo)" }}>{index + 1}</TableCell>
                <TableCell className="whitespace-normal break-words">
                  <span className="text-sm font-semibold" style={{ color: "var(--text-hi)" }} title={l.locationName}>{l.locationName}</span>
                </TableCell>
                <TableCell>
                  <div className="flex items-center gap-1.5">
                    {AreaIcon}
