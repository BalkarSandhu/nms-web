...
import {
  ...
  ReferenceArea, AreaChart, Area, BarChart, Bar, Legend,
} from "recharts";
import { Activity, AlertCircle, Clock, Gauge, Signal, TrendingUp, TrendingDown} from "lucide-react";
import {
  useDeviceTelemetry,
  type TimeRangeKey,
  type Granularity,
...
export default function DeviceHistoryView({ deviceId, deviceName }: DeviceHistoryViewProps) {
  ...
  // Bucketed status series — for a single device the "stack" represents the
  // share of checks in each bucket that were online vs offline (0..100%).
  // Matches the visual of the Area dashboard's "Device status over time".
  const deviceStatusSeries = useMemo(() => {
    ...
    return [...buckets.entries()]
      .sort((a, b) => a[0] - b[0])
      .map(([k, b]) => ({
        timeLabel: new Date(k).toLocaleString(),
        online:  b.total ? +((b.online  / b.total) * 100).toFixed(1) : 0,
        offline: b.total ? +((b.offline / b.total) * 100).toFixed(1) : 0,
      }));
  }, [history, granularity]);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Device status over time — stacked area, same visual as Area dashboard */}
      <Card className="shadow-lg border border-slate-700 bg-slate-800 text-slate-100">
        <CardHeader className="pb-3">
          <CardTitle className="text-base text-white flex items-center gap-2">
            ...
            <span className="ml-auto text-[11px] font-normal text-slate-400">
              share of checks in each bucket
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {deviceStatusSeries.length === 0 ? (
            <div className="h-64 flex items-center justify-center text-slate-400 text-sm">No data</div>
          ) : (
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={deviceStatusSeries} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  <defs>
                    <linearGradient id="dhv-online" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%"   stopColor="#10b981" stopOpacity={0.65} />
                      <stop offset="100%" stopColor="#10b981" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="dhv-offline" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <YAxis
                    ...
                    unit="%"
                    label={{ value: "Share of checks", angle: -90, position: "insideLeft", style: { fill: "#94a3b8", fontSize: 11 } }}
                  />
                  <Tooltip
                    contentStyle={{ backgroundColor: "#0f172a", borderColor: "#334155", color: "#e2e8f0", fontSize: 12 }}
                    labelStyle={{ color: "#cbd5e1" }}
                  ...
                  <Area type="monotone" dataKey="online"  stackId="1" stroke="#10b981" fill="url(#dhv-online)"  strokeWidth={2} name="Online" />
                  <Area type="monotone" dataKey="offline" stackId="1" stroke="#ef4444" fill="url(#dhv-offline)" strokeWidth={2} name="Offline" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          )}
