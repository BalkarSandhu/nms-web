  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 px-4 w-full h-full">
        ...
        {/* Devices / Locations radios — Downtime page, just before Logout */}
        {isDowntime && (
          <>
            <DowntimeViewToggle />
            <Separator
...
/* Downtime controls — clock-icon timeline dropdown + per-area report.
   • An area card open (?area=) → "Report" generates that area's PDF on click.
   • All-areas page → "Report" opens a dropdown of areas; picking one
     downloads that area's PDF. Shown only on the Downtime page. */
function DowntimeControls() {
  const { range, setRange } = useHistoryRange()
  const [tlOpen, setTlOpen] = useState(false)
  const [areaOpen, setAreaOpen] = useState(false)
  const [busy, setBusy] = useState(false)

  const runDowntimeReport = (areaId) => {
    setBusy(true)
    // Placeholder for downtime report generation logic
    ...
    <div className="flex items-center gap-2">
      <Popover open={tlOpen} onOpenChange={setTlOpen}>
        <PopoverTrigger asChild>
          ...
          >
            <Clock className="size-3.5" />
            <span className="hidden sm:inline">{HISTORY_RANGES.find((r) => r.key === range)?.label}</span>
          </button>
        </PopoverTrigger>
        <PopoverContent
          align="end"
          className="w-40 p-1 max-h-[60vh] overflow-y-auto"
          style={{
            background: 'var(--bg-panel)',
            border: '1px solid var(--border-soft)',
            color: 'var(--text-hi)',
      ...
      <Popover open={areaOpen} onOpenChange={setAreaOpen}>
        <PopoverTrigger asChild>
          <button
            type="button"
            disabled={busy}
            title="Download a downtime report"
            className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-md text-xs font-semibold transition-colors disabled:opacity-50"
            style={{
              background: 'var(--brand-soft)',
              border: '1px solid var(--border-brand)',
